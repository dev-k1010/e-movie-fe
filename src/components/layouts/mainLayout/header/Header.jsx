...
function Header() {
  ...
  const headerMobile = () => {
    return <>
      <div >
        <div className={`relative h-screen border-2 border-cyan-800 bg-slate-500`}>
          <Button
            type="text"
            icon={collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />}
            onClick={() => setCollapsed(!collapsed)}
            className="absolute top-0 right-0 flex items-center justify-center text-2xl w-16 h-16"
          />
          <Sider
            trigger={null}
            collapsible
            collapsed={collapsed}
          // className={`h-full transition-all duration-300 ${collapsed ? 'w-16 hidden' : 'w-1/3 block'}`}
          >
            <Menu
              className=" h-screen "
              theme="dark"
              ...
              items={[
                {
                  key: '1',
                  label: (
                    <div className="w-20 h-20 flex justify-center items-center space-x-2">
  ...
  return (
    <div>
      <div style={{ zIndex: 10000 }}>{headerMobile()}</div>
      <header
        style={{ zIndex: 10000 }}
        className={`Header hidden fixed top-0  w-full ${!navbar ? "bg-black   shadow-sm shadow-black/50 " : " bg-black/60 "
          }
        }`}
      >
        <div>
          <div className="w-full justify-between bg-transparent lg:items-center lg:flex py-5">
            ...
            <div>
              <div
                className={`flex-1 justify-self-center py-3 lg:block lg:py-0 ${navbar ? "block" : "hidden"
                  }`}
              >
