...
const initialState = {
  ...
  errListBanner: undefined,
  // List phim
  listPhim: [],
  isFetchListPhim: false,
  errListPhim: undefined,
  // List phân trang
  listPhimPhanTrang: [],
  isFetchListPhimPhanTrang: false,
  errListPhimPhanTrang: undefined,
  // Thêm phim
  themPhim: null,
  isFetchThemPhim: false,
  errThemPhim: undefined,
  // Xóa phim
  delPhim: null,
  isFetchDelPhim: false,
  errDelPhim: undefined,
  // Sữa phim
  upDatePhim: null,
  isFetchUpdatePhim: false,
  errUpdatePhim: undefined,
};
  ...
  createSlice({
    ...
    extraReducers: (builder) => {
      builder
        ...
        .addCase(layDanhSachPhimPhanTrang.pending, (state, action) => {
          state.isFetchListPhimPhanTrang = true;
        })
        .addCase(layDanhSachPhimPhanTrang.fulfilled, (state, action) => {
          state.listPhimPhanTrang = action.payload;
          state.isFetchListPhimPhanTrang = false;
        })
        .addCase(layDanhSachPhimPhanTrang.rejected, (state, action) => {
          state.errListPhimPhanTrang = action.payload;
          state.isFetchListPhimPhanTrang = false;
        })
...
export const layDanhSachBanner = createAsyncThunk(
...
export const layDanhSachPhimPhanTrang = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    const { soTrang, soPhanTuTrenTrang } = data;
    try {
      const result = await quanLyPhimServices.layDanhSachPhimPhanTrang(
        soTrang,
        soPhanTuTrenTrang
      );
      return result.data.content;
